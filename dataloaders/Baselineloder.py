...
def get_Baseline(data_dir, n_data, preprocessing, reduce_dim, task, sampling_rate):
    """
    Args:
    ...
    if task == "task1":
        n_points = int(np.round(sampling_rate[0] * grid_lat.numel()))
    ...
    return grid_lat, grid_lon, t, gst, indices, normalizer
...
class Baseline(Dataset):
    def __init__(self, 
        ...
        self.grid_lat, self.grid_lon, self.time_t, self.temperature = assemble_xy(grid_lat, grid_lon, time_t, temperature, indices)
        # free memory
        del grid_lat, grid_lon, time_t, temperature, indices

    def __len__(self):
        return self.temperature.shape[0]

    def __getitem__(self, idx):
        lat  = self.grid_lat[idx]
        lon  = self.grid_lon[idx]
        t    = self.time_t[idx]
        temp = self.temperature[idx]
        
        # input lat, lon, time | output temperature
        # size (n_points, 3) | (n_points, 1)
        input_x = torch.cat((torch.unsqueeze(lat, dim=1), torch.unsqueeze(lon, dim=1), torch.unsqueeze(t, dim=1)), dim=1)
...
class vizBaseline(Dataset):
    ...
    def __getitem__(self, idx):
        lat = self.grid_lat
        lon = self.grid_lon
        t   = torch.ones_like(lat) * self.time_t[idx]
        temp = self.temperature[idx]

        # input lat, lon, time | output temperature
        # size (n_points, 3) | (n_points, 1)
        input_x = torch.cat((lat.reshape(-1, 1), lon.reshape(-1, 1), t.reshape(-1, 1)), dim=1)
